...
現場的動作是「誰拿了點誰」。所以晶片預設是**還沒領**（灰色虛線框），點一下變成
**已領**（綠色打勾）。判定在 `public/app.js` 的 `_caseDelivered()` /
`_memberDelivered()` / `_batchDone()`，伺服器 `expectedForDate()` 讀同一份
`day_state` 的 `staff` / `cases`（點過的人）。**要在別處判斷有沒有出餐，
從這些函式讀，不要自己再寫一次。**

有禁忌註記（`contraindications`）的個案一樣要點，只是會先跳核對視窗。那是安全閘門。

#### 為什麼還留著一條退路

2026-08 原本就是這套「逐一勾取」，而且**失敗過**：一個十人的員工日加上個案
要按十四次，實際資料顯示 8/20、8/21、8/24、8/25、8/27 幾乎每個工作日都靠隔天的
自動補扣收尾，其中兩天整批十杯完全沒人勾。後來才改成例外管理（預設已出餐）。

2026-09-03 現場要求改回「誰拿了點誰」，因為例外管理反直覺。改回去了，但把當初
撐住庫存的那張網留著：

> **那一天完全沒有人點過任何一個 → 退回舊規則，照出勤與出單補扣。**

`dayExceptions()` 的 `anyTap` 就是這個判斷，`/api/consumption/expected` 會用
`rule` 欄位講明白它用了哪一套。這條退路同時保護了所有歷史日期 ——
那些日子的 `staff` / `cases` 是空的，沒有它整天會變成 0 杯。

**漏點的方向是安全的**：漏點 → 那杯算沒出 → 庫存少扣 → 帳面比實際多 → 盤點看得出來。
反過來（多扣）才是查不出來的那種錯。

扣庫存另有一道時間閘門（`_timePassed()`），過了出餐時間才扣。
